...
import { facultyMentor } from "../../data/studentDashboard";
...
function ActiveRosterTab() {
  const { members, invites, filledCount, facultyInviteStatus, facultyInviteEmail, revokeInvite, openDrawer, removeMember, isLead } = useTeam();
        ...
        <div className="mb-4 flex items-center justify-between">
          <div>
            <h3 className="text-sm font-bold text-brand-deep">
              Confirmed Roster Members ({filledCount} of 6)
            </h3>
            <p className="text-xs text-brand-muted">
              Verified students officially committed to SIH Problem Statement FR-08.
            </p>
          </div>
          <span className="inline-flex items-center gap-1.5 rounded-lg border border-brand-approved/20 bg-brand-approved/10 px-2.5 py-1 text-xs font-semibold text-brand-approved">
            <CheckIcon className="h-3.5 w-3.5" /> All {filledCount} KYC &amp; PRN Verified
          </span>
        </div>
      ...
      <section className="border-t border-brand-softline pt-4">
        <div className="mb-3.5 flex flex-wrap items-center justify-between gap-2">
          <div>
            ...
            <p className="text-xs text-brand-muted">
              Official internal guide supervising team Problem Statement FR-08 (Independent of 6-student cap).
            </p>
          </div>
          ...
          {facultyInviteStatus === "none" ? (
            <div className="flex items-center gap-3.5">
              ...
              <div>
                ...
                <p className="mt-0.5 text-[11px] text-brand-muted">
                  Invite an institute faculty member to guide your track.
                </p>
              </div>
            </div>
          ) : (
            <div className="flex items-center gap-3.5">
              <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-brand-approved text-xs font-bold text-white shadow-sm">
                {facultyMentor.initials}
              </div>
              <div>
                <div className="flex flex-wrap items-center gap-2">
                  <span className="text-sm font-bold text-brand-deep">{facultyMentor.name}</span>
                  <span className="rounded-md border border-brand-approved/20 bg-brand-approved/10 px-2 py-0.5 text-[10px] font-bold text-brand-approved">
                    {facultyMentor.designation}
                  </span>
                  <span className="rounded border border-brand-softline bg-white px-1.5 py-0.5 font-mono text-[10px] text-brand-muted">
                    EMP-ID: {facultyMentor.empId}
                  </span>
                </div>
                <p className="mt-0.5 text-[11px] text-brand-muted">
                  {facultyMentor.nameLabel} •{" "}
                  <span className="font-mono text-brand-deep">
                    {facultyInviteEmail && facultyInviteStatus === "sent" ? facultyInviteEmail : facultyMentor.email}
                  </span>
                </p>
              </div>
            </div>
...
function IncomingRequestsTab() {
  const { requests, requestResults, filledCount, pendingRequestCount, approveRequest, rejectRequest } = useTeam();
  const seatsLeft = 6 - filledCount;
  ...
  return (
    <div className="space-y-4">
      <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
        <div>
          <h3 className="text-sm font-bold text-brand-deep">
            Student Requests to Join &quot;Nex-Aura&quot;
          </h3>
          <p className="text-xs text-brand-muted">
            Unassigned candidates seeking to fulfill your remaining squad slots for SIH 2026.
          </p>
        </div>
      ...
      <div className="flex items-center gap-2 rounded-xl border border-brand-softline bg-brand-cream p-3 text-xs text-brand-muted">
        ...
        Accepting either request will fill Slot 5 or 6 and immediately register the candidate into Team Nex-Aura.
      </div>
    </div>
  );
}
