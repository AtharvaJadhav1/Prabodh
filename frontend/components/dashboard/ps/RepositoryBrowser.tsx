...
import { siFolders, siStatements, type SiStatement } from "../../../data/studentDashboard";
...
export default function RepositoryBrowser() {
  ...
  const filtered = useMemo(() => {
    let results = siStatements.filter((s) => matchesSearch(s, search) && matchesFilter(s, activeFilter));
    if (activeFolder) {
    ...
    return results;
  }, [search, activeFilter, activeFolder]);
            ...
            return (
              ...
              >
                ...
                <div className="mt-2 flex items-center justify-between text-brand-muted">
                  <span className="font-mono text-xs font-semibold">{folder.count} problem statements</span>
                  <svg
            ...
            >
              ...
              <div className="flex shrink-0 flex-col items-end gap-2 pt-3 sm:pt-0">
                {isLead ? (
                  <button
                    type="button"
                    className="inline-flex items-center gap-1.5 rounded-xl bg-brand-primary px-3 py-2 text-xs font-bold text-white shadow-md shadow-brand-primary/25 transition-all duration-150 hover:bg-brand-hover active:scale-[0.99]"
                  >
