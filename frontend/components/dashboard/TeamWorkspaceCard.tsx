...
import { team, type Member } from "../../data/studentDashboard";
...
export default function TeamWorkspaceCard() {
  const { members, filledCount, openDrawer, invites, isLead } = useTeam();
  const percent = Math.round((filledCount / team.capacity) * 100);
  const pendingInvites = invites.length;
  const availableSlots = team.capacity - filledCount;
  ...
  return (
    <section className="rounded-2xl border border-brand-softline bg-white p-5 shadow-[0_2px_8px_rgba(91,46,16,0.04)] sm:p-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div className="flex min-w-0 items-center gap-3.5">
          ...
          <div className="min-w-0">
            ...
            <p className="truncate text-xs font-medium text-brand-muted">
              Team ID: <span className="font-bold text-brand-charcoal">{team.id}</span> • {team.branch}
            </p>
          </div>
        </div>
        <span
          className={`shrink-0 self-start rounded-full border px-3 py-1 text-xs font-bold sm:self-center ${
            filledCount === team.capacity
        ...
        >
          {filledCount} of {team.capacity} Members Finalized
        </span>
      </div>
