...
import { STUDENT_ROLE_KEY, student as studentInfo } from "../../data/studentDashboard";
...
export default function Sidebar({ mobileOpen, onCloseMobile }: SidebarProps) {
  ...
  const { filledCount, pendingRequestCount, role } = useTeam();
        ...
        <div className="border-b border-brand-softline px-4 py-4">
          <div className="rounded-2xl border border-brand-softline bg-white p-4">
            ...
            <div className="mt-1.5 flex items-center justify-between text-sm">
              <span className="font-extrabold text-brand-deep">Team Nex-Aura</span>
          ...
          <div className="flex flex-col gap-3">
            <div className="flex items-center gap-3 rounded-xl border border-brand-softline bg-white p-3">
              ...
              <div className="min-w-0">
                <p className="truncate text-sm font-bold text-brand-deep">{studentInfo.name}</p>
                <p className="text-xs font-medium text-brand-muted">
                  {role} • {studentInfo.prn}
                </p>
              </div>
            </div>
            <button
              ...
              onClick={() => {
                try {
                  window.localStorage.removeItem(STUDENT_ROLE_KEY);
                } catch {
