...
export default function DispatchInviteCard() {
  const { isLead, sendInvite, filledCount, invites } = useTeam();
  ...
  const handleSend = () => {
    ...
    if (seatsLeft <= 0 || filledCount + invites.length >= team.capacity) {
      ...
      {isLead ? (
        ...
        <div className="mt-3">
          ...
          <div className="mt-3 flex items-center gap-2 rounded-xl border border-brand-softline bg-brand-cream p-3 text-xs font-semibold text-brand-muted">
            ...
            {filledCount}/{team.capacity} members finalized • {invites.length} pending invite
            {invites.length !== 1 ? "s" : ""}
          </div>
        </div>
      )}
