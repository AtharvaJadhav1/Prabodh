      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="mt-2 text-sm leading-relaxed text-brand-muted">
            Sign in with Clerk. Admin, institute mentor, and industry mentor dashboards follow your account role.
          </p>
        </div>
        <ClerkSignInPanel path="/login/faculty" />
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-2 text-sm leading-relaxed text-brand-muted">
          Local dev sign-in. Role is taken from your seeded account.
        </p>
      </div>
      ...
      <form
        ...
        onSubmit={async (e) => {
          ...
          setError("");
          setLoading(true);
          const email = String(new FormData(e.currentTarget).get("faculty-email") ?? "");
          try {
            const user = await login(email);
            router.push(dashboardForRole(user.platformRole));
          } catch (err) {
      ...
      >
        ...
        <div className="space-y-1.5">
          <label htmlFor="faculty-password" className="block text-xs font-bold uppercase tracking-wider text-brand-deep">
            Password
          </label>
