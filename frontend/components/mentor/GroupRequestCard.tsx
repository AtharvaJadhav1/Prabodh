...
export default function GroupRequestCard({ request, atCap, onAccept, onDecline }: Props) {
  ...
  const trackBadge =
    request.track === "Priority Track" ? (
      <span className="inline-flex items-center gap-1 rounded-full border border-brand-amber/30 bg-brand-amber/10 px-2.5 py-0.5 text-[11px] font-bold text-brand-primary">
  ...
  return (
    <>
      <div className="rounded-xl border border-brand-sand bg-white p-5 shadow-sm transition-all hover:border-brand-primary/30 hover:shadow-md">
        ...
        <div className="mb-3 flex flex-wrap items-center gap-x-5 gap-y-1.5 text-xs text-brand-muted">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span className="font-mono text-brand-muted">{request.leaderPrn}</span>
          </div>
        ...
        <div className="mb-3 flex flex-wrap items-center gap-2 text-[11px]">
          <span className="inline-flex items-center gap-1 rounded border border-brand-sand bg-brand-cream px-2 py-0.5 font-medium text-brand-deep">
            ...
            {request.memberCount}/{request.memberCount}
          </span>
          <span className="inline-flex items-center gap-1 rounded border border-brand-sand bg-brand-cream px-2 py-0.5 font-medium text-brand-deep">
        ...
        <p className="mb-1.5 text-xs font-bold text-brand-charcoal">
          <span className="text-brand-primary">{request.groupId}:</span> {request.problemTitle}
        </p>
      ...
      <ConfirmDialog
        ...
        message={
          confirmMode === "accept"
            ? `Accept mentorship for ${request.teamName}? You'll be assigned as their ${request.allocatedRole}. This is an ${request.track.toLowerCase()} request.`
